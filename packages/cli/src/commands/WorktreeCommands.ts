...
import { WorktreeCli } from "../services/WorktreeCli.js";
...
export class WorktreeCommands {
  static build() {
    return Command.make("gw").pipe(
      ...
      Command.withSubcommands([
        WorktreeCommands.buildList(),
        WorktreeCommands.buildCreate(),
        WorktreeCommands.buildEdit(),
      ]),
    );
  }
  ...
  private static buildList() {
    return Command.make("list").pipe(
      ...
      Command.withHandler(() =>
        Effect.gen(function* () {
          const cli = yield* WorktreeCli.make();
          yield* cli.list();
        }),
      ),
    );
