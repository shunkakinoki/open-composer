...
/**
 * Represents a styled text token with ANSI attributes
 */
...
/**
 * Terminal output is an array of lines
 */
...
/**
 * Internal cell representation with parsed attributes
 */
class Cell {
...
/**
 * Serialize a headless Terminal to AnsiOutput format
 * This creates a structured representation that can be rendered in Ink
 *
 * Note: Reads from the scrollback buffer starting at viewportY (not from 0)
 * to show what's currently visible in the terminal viewport
 */
export function serializeTerminalToObject(terminal: Terminal): AnsiOutput {
  ...
  // Read from the viewport (viewportY + rows)
  for (let y = 0; y < terminal.rows; y++) {
    ...
    for (let x = 0; x < terminal.cols; x++) {
      ...
      // When attributes change, flush the current token
